...
function initProductForm() {
  ...
  document.getElementById("fCategory")?.addEventListener("change", renderSizeCheckboxes);
}
...
function deleteProduct(id) {
  openConfirm("¿Eliminar este producto? Esta acción no se puede deshacer.", () => {
...
function resetToSample() {
  openConfirm("¿Restaurar los productos de ejemplo? Se perderán todos los cambios.", () => {
    localStorage.removeItem(CONFIG.storageKey);
    initData();
    adminToast("✓ Productos de ejemplo restaurados", true);
    if (currentView === "products") renderAdminProducts();
    if (currentView === "dashboard") loadDashboard();
  });
}

function clearAllProducts() {
  openConfirm("¿Eliminar TODOS los productos? Esta acción es irreversible.", () => {
    saveProducts([]);
